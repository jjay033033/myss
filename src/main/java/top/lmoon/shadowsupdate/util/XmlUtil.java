...
public class XmlUtil {
	 ...
	 * 
	 * @param rowList
	 *            结果记录列表：列表元素需为Map类型，Map的key值将作为结点名称
	 * @return xml
	 */
	@Deprecated
	public static String getXml(List rowList) {
		...
		while (it.hasNext()) {
			...
			for (Map.Entry entry : entrys) {
				result.append("<" + entry.getKey() + "><![CDATA[" + entry.getValue() + "]]></" + entry.getKey() + ">");
			}
			result.append("</row>");
		}
	 ...
	 *            被排字段的数据类型，目前被支持的类型有： Integer.class/int.class,
	 *            其他类型都会被当成String类型来处理
	 * @param inc
	 *            是否按升序
	 */
	@Deprecated
